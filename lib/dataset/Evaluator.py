...
class Evaluator:
    ...
    def scale_mesh_to_match(self):
        """
        将预测的 mesh 缩放，使其与真实 mesh 的大小一致
        """
        # 计算真实 mesh 的范围
        verts_gt_min = torch.min(self.verts_gt, dim=0).values
        verts_gt_max = torch.max(self.verts_gt, dim=0).values
        gt_range = verts_gt_max - verts_gt_min # cuda:0

        # 计算预测 mesh 的范围
        verts_pr_min = torch.min(self.verts_pr, dim=0).values
        verts_pr_max = torch.max(self.verts_pr, dim=0).values
        pr_range = verts_pr_max - verts_pr_min # cpu

        # 确保 gt_range 和 pr_range 在同一个设备上
        gt_range = gt_range.to(self.device)
        pr_range = pr_range.to(self.device)

        # 计算缩放因子
        scale_factor = gt_range / pr_range
        # print(f"self.verts_pr 所在的设备: {self.verts_pr.device}")


        # 对预测 mesh 的顶点坐标进行缩放
        self.verts_pr = self.verts_pr * scale_factor.to(self.verts_pr.device)
