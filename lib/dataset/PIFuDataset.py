...
class PIFuDataset():
    ...
    def __init__(self, cfg, split='train', vis=False):
        ...
        self.split = split
        self.root = cfg.root # ./data
        self.bsize = cfg.batch_size # 4
        self.overfit = cfg.overfit

        # for debug, only used in visualize_sampling3D
        self.vis = vis

        self.opt = cfg.dataset # config中dataset参数设置
        self.datasets = self.opt.types # thuman2
        self.input_size = self.opt.input_size # 512
        self.scales = self.opt.scales # [100,0]
        self.workers = cfg.num_threads # 4
        ...
        self.in_geo = [item[0] for item in cfg.net.in_geo] # ['normal_F','normal_B']
        self.in_nml = [item[0] for item in cfg.net.in_nml] # ['image','T_normal_f','T_mormal_B']

        self.in_geo_dim = [item[1] for item in cfg.net.in_geo] # [3,3]
        self.in_nml_dim = [item[1] for item in cfg.net.in_nml] # [3,3,3]
        ...
        self.feat_keys = self.base_keys + [f"smpl_{feat_name}" for feat_name in self.feat_names] # smpl七个特征

        if self.split == 'train':
            self.rotations = np.arange(0, 360, 360 / self.opt.rotation_num).astype(np.int32)
        else:
            self.rotations = range(0, 360, 90)
        ...
        for dataset_id, dataset in enumerate(self.datasets):
            ...
            dataset_dir = osp.join(self.root, dataset) # ./data/thuman2

            mesh_dir = osp.join(dataset_dir, "scans")
            smplx_dir = osp.join(dataset_dir, "smplx") # ./data/thuman2/smplx
        ...
        self.UV_RENDER='./data/thuman2_3views'
        self.UV_MASK='./data/thuman2_3views/UV_MASK'
        self.UV_POS='./data/thuman2_3views/UV_POS'
        self.UV_NORMAL='./data/thuman2_3views/UV_NORMAL'
        self.IMAGE_MASK='./data/thuman2_3views/MASK'
        self.PARAM='./data/thuman2_3views/PARAM'
        self.depth='./data/thuman2_3views'
    ...
    def __getitem__(self, index):
        ...
        # render_folder = "/".join([dataset + f"_3views", subject])
        render_folder = "/".join([dataset + f"_4views",subject])
        if dataset=='thuman2':
        ...
        uv_render_path=os.path.join(self.UV_RENDER,subject,'uv_color', '', '%d_%d_%02d.jpg'%(rotation,0,0))
        uv_mask_path = os.path.join(self.UV_RENDER, subject,'uv_mask','%02d.png' % (0))
        uv_pos_path = os.path.join(self.UV_RENDER, subject,'uv_pos', '%02d.exr' % (0))
        uv_normal_path = os.path.join(self.UV_RENDER, subject,'uv_normal', '%02d.png' % (0))
        image_mask_path=os.path.join(self.IMAGE_MASK,subject,'%d_%d_%02d.png'%(rotation,0,0))
        param_path=os.path.join(self.PARAM, subject,'%d_%d_%02d.npy'%(rotation,0,0))
        depth_path=os.path.join(self.depth,subject,"depth_F",'%03d.png'%(rotation))
        ...
        elif dataset == 'CLOTH4D':
            data_dict.update({
                'mesh_path': osp.join(self.datasets_dict[dataset]["mesh_dir"], f"{subject}/{subject[4:]}.obj"),  # './data/CLOTH4D/scans/s10_00_000034_uv/00_000034_uv.obj'
                'smpl_param':
                osp.join(self.datasets_dict[dataset]["smpl_dir"], f"{subject}.pkl"),
            })
    ...
    def compute_smpl_verts(self, data_dict, noise_type=None, noise_scale=None):
        ...
        smplx_param = load(data_dict['smpl_param'])  # 读取cloth4d的pkl文件

        # smplx_param = np.load(data_dict['smpl_param'], allow_pickle=True) # 修改为smpl\
        ...
        print(f"add noise to smpl--smplx_pose{smplx_pose.shape}, smplx_betas{smplx_betas.shape}")

        smplx_out, _ = load_fit_body(fitted_path=data_dict['smpl_param'], # 修改为smpl
                                     scale=self.datasets_dict[dataset]['scale'],
                                     smpl_type='smpl', # 修改为smpl
                                     smpl_gender='male',
    ...
    def load_smpl(self, data_dict, vis=False, densely_sample=False):
        ...
        print(f"smpl verts:{smplx_verts.shape}, faces:{smplx_faces.shape}, cmap:{smplx_cmap.shape}")
    ...
    def get_sampling_geo(self, data_dict, is_valid=False, is_sdf=False):
        ...
        elif self.datasets[0]=='cape' or self.datasets[0]=='CLOTH4D':
