...
def get_smpl_model(model_type, gender):
    return smpl.create(**model_init_params)
...
def load_fit_body(fitted_path,
    ...
    param = load(fitted_path)
    # for key in param.keys():
    #     param[key] = torch.as_tensor(param[key])
        
    param = {k: torch.as_tensor(v, dtype=torch.float32) for k, v in param.items()}
    ...
    if noise_dict is not None:
        ...
        model_forward_params["betas"] = torch.tensor(model_forward_params["betas"]).unsqueeze(0)
        model_forward_params["body_pose"] = torch.tensor(model_forward_params["body_pose"]).unsqueeze(0)
        
        
    model_forward_params = {k: torch.as_tensor(v, dtype=torch.float32) for k, v in model_forward_params.items()}
        
    for k in model_forward_params.keys():
        print(f"{k}_{model_forward_params[k].shape}")
...
def load_fit_body_new(fitted_path,
                  scale,
                  noise_dict=None):
    
    smpl_model_path = 'data/smpl_related/models/smpl'
    smpl = SMPL(
        model_path=smpl_model_path,
        gender='female',        # 也可以是 'female' 或 'neutral'
    ...
    betas = torch.tensor(param['betas'], dtype=torch.float32)
    body_pose = torch.tensor(param['body_pose'], dtype=torch.float32).reshape(1, 23, 3)
    global_orient = torch.tensor(param['global_orient'], dtype=torch.float32).reshape(1, 1, 3)

    if noise_dict is not None:
        betas = torch.tensor(noise_dict['betas'], dtype=torch.float32).reshape(1, 10)
        body_pose = torch.tensor(noise_dict['body_pose'], dtype=torch.float32).reshape(1, 23, 3)
